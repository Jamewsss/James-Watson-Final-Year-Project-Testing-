...
def process_dataA(df, i):
    ...
    scaler = load('scaler_phaseA.joblib')
    X_scaled = scaler.transform(new_X)
    pca = load('pca_phaseA.joblib')
    ...
    def adjust_major_axis_width(X, desired_width):
        ...
        #inverse transform to get back to the original data format
        ...
        pca = PCA(n_components=2)
        X_adjusted_pca = pca.fit_transform(X_adjusted)
        # Apply the correction scaling
        #X_adjusted[:, 0] *= correction_scale_x 
        #X_adjusted = pca.inverse_transform(X_adjusted_pca)
        ...
        #alignment_angle = -angle_of_major_axis-2.4  # Rotate back by the negative of current angle
